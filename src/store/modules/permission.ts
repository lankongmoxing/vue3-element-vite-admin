...
const mutations = {
  ...
  SET_HAS_BUILDED(state: any, val: Boolean) {
...
const actions = {
  ...
  removeBuildFlag(context: any) {
    context('SET_HAS_BUILDED', false)
  },
  ...
  CurrentIsAuths(context: any, routers: any) {
    ...
    if (currentMenuName) {
      for (let i = 0; i < routers.length; i++) {
        ...
        } else if (!status && routers[i].children && (routers[i].children as any).length > 0) {
          (routers[i].children as any).map((every: any) => {
            if (!status && every.name && every.name === currentMenuName && (!every.meta.hidden || every.meta.isDetail)) {
              if (every.meta.isDetail) {
                const isParentMenu = routers[i].children.find((e: any) => {
                  return e.name === every.meta.parentTitle
                })
                status = true
                currentIsAuth = isParentMenu ? !isParentMenu.meta.hidden : false
                return currentIsAuth
              } else {
                currentIsAuth = true
                return currentIsAuth
